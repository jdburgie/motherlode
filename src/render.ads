...
package Render is
   ...
   type Framebuffer_Access is access all Frame_Buffer;
   ...
   FB1 : aliased Frame_Buffer := (others => 0);
   FB2 : aliased Frame_Buffer := (others => 0);
   Flip : Boolean := True;

   procedure Refresh_Screen (Acc : not null Framebuffer_Access);
